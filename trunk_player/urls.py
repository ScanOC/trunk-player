...
urlpatterns = [
    ...
    url(r'^(tg|scan|unit)/(.*)/$',login_required(TemplateView.as_view(template_name='radio/player_main.html'))),
    url(r'^inc/(.*)/$',views.incident, name='incident'),
    url(r'^scan/$', RedirectView.as_view(url='/scan/default/', permanent=False)),
    url(r'^userscan/$', login_required(TemplateView.as_view(template_name='radio/player_userscan.html'))),
    ...
    url(r'^register/$', views.Generic, {'page_name': 'index'}, name='register'),
    ...
    #url(r'^api_v2/', include('talkgroup.urls')),
